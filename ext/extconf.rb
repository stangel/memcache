...
$CXXFLAGS = (RUBY_PLATFORM =~ /darwin/i) ? " -std=gnu++98 -fPIC" : " -std=gnu++0x -fPIC"
  ...
  Dir.chdir(HERE) do
    if false and File.exist?("lib")
      ...
      Dir.chdir(BUNDLE_PATH) do
        puts(cmd = "patch -p1 < ../libmemcached-1.0.18_pointer-fix.diff 2>&1")
        raise "'#{cmd}' failed" unless system(cmd)
...
$CPPFLAGS += " -DRUBY_2X" if RUBY_VERSION =~ /2./
$CPPFLAGS += " -DRUBY_3X" if RUBY_VERSION =~ /3./
